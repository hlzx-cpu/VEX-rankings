...
<p align="center">
  <strong>🤖 VEX U (VURC) 2026-2027 Override 赛季全自动 Elo 排名看板</strong>
</p>
<p align="center">
...
>
> 数据建议每 6 小时由 GitHub Actions 自动更新，无需安装任何环境，打开即用。
...
所有数据通过 [events.vex.com](https://events.vex.com) 公开页面与公开 JSON 接口获取，涵盖 Teams、Matches、Skills 三类数据；当前低频更新方案不需要自备 API Token。
...
# ── 每 12 小时运行一次 ──
# 0 */12 * * * cd /你的项目绝对路径/VEX-rankings && /usr/bin/python3 data_fetcher.py >> /tmp/vex-rankings.log 2>&1
...
| #   | 文件              | 位置                          | 当前值                     | 说明                         |
| --- | ----------------- | ----------------------------- | -------------------------- | ---------------------------- |
| 1   | `data_fetcher.py` | 顶部配置                      | `SEASON_YEAR = 2026`       | **最关键**：决定抓取哪个赛季 |
| 2   | `data_fetcher.py` | `GAME_NAME` / HTML 文案       | `Override`                 | 网页标题与空数据提示         |
| 3   | `app.py`          | `dash.Dash(title=...)`        | `VURC 2026-2027 Override 战绩看板` | Dash 看板标题        |

> 💡 全局搜索当前赛季年份，并更新 `SEASON_YEAR` / `GAME_NAME` 即可。
...
**当前不需要。** 本项目改用 events.vex.com 的公开低频数据源，GitHub Actions 可不配置 `ROBOTEVENTS_TOKEN`。
...
公开接口仍可能限流，因此推荐低频运行（例如每 6 小时）。脚本会主动限制请求速率并在 429 时退避重试。
